...
namespace ACAD_BieuDoKeHoachVatTu
{
    ...
    public class ACADLib : IExtensionApplication
    {
        ...
        [CommandMethod("GHIFILE")]
        public void SoHoaToanDo()
        {

            string filename = Library.GetStringFromUser("Enter filename:");

            string Xtitle = Library.GetStringFromUser("X label:");
            string Ytitle = Library.GetStringFromUser("Y label:");

            //Truc X
            double Xmin = Library.GetDoubleFromUser("X-AXIS Minimum:");
            double Xmax = Library.GetDoubleFromUser("X-AXIS Maximum");

            //Truc Y
            double Ymin = Library.GetDoubleFromUser("Y-AXIS Minimum:");
            double Ymax = Library.GetDoubleFromUser("Y-AXIS Maximum");

            Point3d pO = Library.GetPointsFromUser("Pick point O:");

            Point3d pX = Library.GetPointsFromUser("Pick endpoint of X-axis");
            Point3d pY = Library.GetPointsFromUser("Pick endpoint of Y-axis");

            Document doc = Application.DocumentManager.MdiActiveDocument;

            Editor ed = doc.Editor;

            var csvFile = $"D:/{filename}.csv";

            File.AppendAllText(csvFile, $"{Xtitle},{Ytitle}\r\n");

            foreach (Point3d pt in list_pts)
            {
                double Xval = linear(pt.X, pO.X, pX.X, Xmin, Xmax);
                double Yval = linear(pt.Y, pO.Y, pY.Y, Ymin, Ymax);

                File.AppendAllText(csvFile, $"{Xval},{Yval}\r\n");

                //ed.WriteMessage(Xval + "\t" + Yval + "\n");

            }
            list_pts = new Point3dCollection();
        }
        ...
        Point3dCollection list_pts = new Point3dCollection();
        [CommandMethod("LV")]
...

                {
...

                    {
...

                        {
                            ...
                            list_pts.Add(new Point3d(pt.X,pt.Y,0));
        ...
        public void ed_PointMonitor(object sender, PointMonitorEventArgs e)
        {
